...
class CamApp(App):
    ...
    def build(self):
        ...
        self.button.size_hint_x = self.web_cam.size_hint_x
    ...
    def face_detect(self, imgpath):
        try:
            dfs = DeepFace.find(imgpath, db_path="train", model_name=models[0])
            if len(dfs) > 0:
    ...
    def verify(self, *args):
        ...
        img_rec, output_data = self.face_detect(save_path)
        if img_rec is not None:
            frame = img_rec
        cv2.imwrite(save_path, frame)

        self.is_verifying = True
        self.current_image = cv2.imread(save_path)
        end = time.time()
        timer = end - start
    ...
    def reset_verification_label(self, *args):
        self.verification_label.color = get_color_from_hex('#000000')
